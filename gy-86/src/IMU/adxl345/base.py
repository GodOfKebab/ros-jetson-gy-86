...
class ADXL345_Base(object):
    ...
    def read_data(self):
        ...
        data_x = self.get_registers(ADXL345_Base.REG_DATAX0, 2)
        x = self._convert(data_x[0], data_x[1])  + self.offsets['ax']
        x *= 9.81

        data_y = self.get_registers(ADXL345_Base.REG_DATAY0, 2)
        y = self._convert(data_y[0], data_y[1]) + self.offsets['ay']
        y *= 9.81

        data_z = self.get_registers(ADXL345_Base.REG_DATAZ0, 2)
        z = self._convert(data_z[0], data_z[1]) + self.offsets['az']
        z *= 9.81
    ...
    def calibrate(self, num_of_calibration_measurements=4500, print_status=False):
        ...
        while (self.calibration_cycle_count < num_of_calibration_measurements) and (not rospy.is_shutdown()):
            ...
            self.total_acc_offset[0] += cal_x - ax
            self.total_acc_offset[1] += cal_y - ay
            self.total_acc_offset[2] += cal_z - az

            if print_status:
                avg_ax = self.total_acc_offset[0] / self.calibration_cycle_count
                avg_ay = self.total_acc_offset[1] / self.calibration_cycle_count
                avg_az = self.total_acc_offset[2] / self.calibration_cycle_count

                s = "[{}/{}][Error Count:{}] ax:{:0.4f}  ay:{:0.4f}  az:{:0.4f}".format(self.calibration_cycle_count,
        ...
        self.offsets = {'ax': self.total_acc_offset[0] / self.calibration_cycle_count,
                        'ay': self.total_acc_offset[1] / self.calibration_cycle_count,
                        'az': self.total_acc_offset[2] / self.calibration_cycle_count}
        self.power_off()
